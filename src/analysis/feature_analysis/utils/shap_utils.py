...
class ShapVisualizer:
    ...
    def _parse_features(self):
        ...
        feat_info = pd.DataFrame([parse(c) for c in self.X_test.columns],
                                 columns=["band", "stat", "month"],
                                 index=self.X_test.columns)
        return feat_info

    def _mean_abs_shap(self, axis=(0, 2)):
        return pd.Series(np.mean(np.abs(self.shap_values), axis=axis),
                         index=self.X_test.columns)
    ...
    def save_class_summary_plots(self, max_display=10, dpi=200):
        ...
        for i, cls in enumerate(self.le.classes_):
            shap.summary_plot(self.shap_values[:, :, i], self.X_test,
    ...
    def plot_top_feature_dependence_per_class(self, dpi=200):
        ...
        for i, cls in enumerate(self.le.classes_):
            ...
            shap.dependence_plot(top_feat, self.shap_values[:, :, i],
                                 self.X_test, show=False)
    ...
    def plot_grouped_importance(self):
        ...
        def group_plot(by, title, ax):
            ...
            ax.set_title(title); ax.set_xlabel("Mean |SHAP|"); ax.set_ylabel("")
    ...
    def plot_heatmaps_grouped(self):
        ...
        def make_heatmap(idx, col, top_bands=None):
            ...
            mat = data.groupby([idx, col]).apply(
                lambda x: shap_mean[x.index].mean()).unstack()
            if "month" in (idx, col):
                if col == "month":
        ...
        for ax, (title, data) in zip(axes, heatmaps):
            ...
            ax.set_title(title); ax.set_xlabel(data.columns.name); ax.set_ylabel(data.index.name)
        plt.tight_layout(); plt.show()