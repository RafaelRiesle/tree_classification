...
class DetectDisturbedTrees:
    ...
    def get_std(self, df_yearly):
        return (
            df_yearly.groupby("id")[self.bands_and_indices]
            .std()
            .add_suffix("_std")
            .reset_index()
        )
    ...
    def get_slope(self, df_yearly):
        ...
        for id_, group in df_yearly.groupby("id"):
            if group["year"].nunique() < 2:
                continue
            slopes = {
                f"{col}_slope": linregress(group["year"], group[col]).slope
                for col in self.bands_and_indices
            }
            slopes["id"] = id_
    ...
    def get_balanced_train_data(self, df_std_slope):
        ...
        return (
            pd.concat([disturbed, healthy_sub])
    ...
    def apply_model(self, model, df_std_slope):
        feature_cols = [
            c for c in df_std_slope.columns if c.endswith(("_std", "_slope"))
        ]
    ...
    def change_disturbed_labels(self, df_final):
        ...
        df_final.loc[
            disturbed_mask & ~df_final["species"].str.endswith("_disturbed"), "species"
        ] = (
            df_final.loc[
        ...
        df_final.loc[
            disturbed_mask & (df_final["is_disturbed"] == False), "is_disturbed"
        ] = True
