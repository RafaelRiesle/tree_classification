...
def evaluate_and_save_model(
    run_id: str, pipeline: GenericPipeline, ensemble_manager: EnsembleModelManager
):
    ...
    metrics = {
        "Train": EnsembleModelManager.compute_metrics(y_train, y_pred_train)[
            "accuracy"
        ],
        "Test": EnsembleModelManager.compute_metrics(y_test, y_pred_test)["accuracy"],
        "Validation": EnsembleModelManager.compute_metrics(y_val, y_pred_val)[
            "accuracy"
        ],
    }
    ...
    print(f"Evaluation for run_id={run_id} completed. Results saved to {run_dir}")
