...
class StatisticalFeatures:
    ...
    def calculate_keyfigures_per_id(self, df, bands_and_indices):
        ...
        monthly_agg = df.groupby(["id", "month"])[bands_and_indices].agg(["mean", "std", "min", "max"])

        monthly_agg.columns = ["_".join([col[0], col[1]]) for col in monthly_agg.columns.values]
        monthly_agg = monthly_agg.reset_index()

        df_train = monthly_agg.pivot(index="id", columns="month", values=monthly_agg.columns[1:])
        ...
        df_train = df_train.drop(columns=[col for col in df_train.columns if "month_" in col])
